...
public class MyClassLoader {
	...
	public <GENERIC> List<GENERIC> load(String pathFile, List<String> classesToLoad) {
		...
		private List<File> findFileJars(String pathFile) {
		   ...
		   File[] fileList = Optional.of(fileJar).map(File::listFiles).orElse(new File[]{});
		   return Arrays.asList(fileList).stream().filter(i->i.getPath().endsWith(extentionJars)).collect(Collectors.toList());
		}
